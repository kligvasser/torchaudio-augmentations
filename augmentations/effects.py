...
class EffectChain:
    ...
    def _apply_sox_effects(
        ...
        out, sr = sox_chain.apply_flow_effects(input_tensor, src_info, target_info)
        return out, sr
    ...
    def apply(
        ...
        target_info: Optional[Dict[str, Union[int, float]]] = None,
    ) -> torch.Tensor:
        """
    ...
    def additive_noise(self, noise_generator: Callable, snr: float):
        ...
        True
        """
        self._chain.append(AdditiveNoise(noise_generator=noise_generator, snr=snr))
        return self
...
def create_method(name):
    ...
    return lambda s, *x: s._append_effect_to_chain(
        name, list(x)
